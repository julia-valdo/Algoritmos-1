...
/* Pre-Condicion:
 * Post-Condicion: dependiendo el valor de viento imprime un mensaje por pantalla.
 */
void mostrar_viento(int* viento){

	if(*viento == VIENTO_ALTO){
		printf("Mucho viento,");
 ...
 * Post-Condicion:
 */
void mostrar_resumen(int viento, int humedad, char animo_legolas, char animo_gimli){
...
void animos(int* viento, int* humedad, char* animo_legolas, char* animo_gimli){
	...
	(*viento) = calcular_viento(dia_mes);
	...
	(*animo_legolas) = letra_animo(puntos_animo_legolas);

	(*animo_gimli) = letra_animo(puntos_animo_gimli);

	mostrar_viento(viento);

	mostrar_humedad(humedad);

	mostrar_animo(animo_legolas);
	printf("Legolas y definitivamente");

	mostrar_animo(animo_gimli);
	printf("Gimli.\n");
