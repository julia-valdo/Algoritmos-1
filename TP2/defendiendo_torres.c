...
const int VIDA_ORCOS = 50;
const int CANT_ORCOS_ESTE = 10;
const int CANT_ORCOS_OESTE = 10;
const int CANT_ORCOS_NORTE = 10;
const int CANT_ORCOS_SUR = 10;
 ...
 */
void cargar_torre(torres_t* torres, configuracion_t* configuracion){

	(*torres).resistencia_torre_1 = RESISTENCIA_T;
	if((*configuracion).resistencia_torre_1 != CONFIG_ESTANDAR){
		(*torres).resistencia_torre_1 = (*configuracion).resistencia_torre_1;
	}
	(*torres).resistencia_torre_2 = RESISTENCIA_T;
	if((*configuracion).resistencia_torre_2 != CONFIG_ESTANDAR){
		(*torres).resistencia_torre_2 = (*configuracion).resistencia_torre_2;
	}
	(*torres).enanos_extra = ENANOS_EXTRA;
	if((*configuracion).enanos.cantidad_extra != CONFIG_ESTANDAR){
		(*torres).enanos_extra = (*configuracion).enanos.cantidad_extra;
	}
	(*torres).elfos_extra = ELFOS_EXTRA;
	if((*configuracion).elfos.cantidad_extra != CONFIG_ESTANDAR){
		(*torres).elfos_extra = (*configuracion).elfos.cantidad_extra;
	}
}
...
void mostrar_terreno_grande(juego_t juego, defensor_t defensores[MAX_DEFENSORES], char terreno[MAX_TERRENO_3_4][MAX_TERRENO_3_4], coordenada_t camino_1[MAX_LONGITUD_CAMINO], int tope_camino_1, coordenada_t camino_2[MAX_LONGITUD_CAMINO], int tope_camino_2){
